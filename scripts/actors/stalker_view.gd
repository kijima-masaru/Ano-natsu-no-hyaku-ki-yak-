...
static func draw_debug(stalker: Node2D, state_name: String, chasing: bool, facing: Vector2i, hearing_radius: float, vision_range: float, vision_half_angle: float) -> void:
	stalker.draw_arc(Vector2.ZERO, maxf(hearing_radius, 0.5 * GameConstants.TILE_SIZE), 0.0, TAU, 32, Palette.with_alpha(Palette.FLUORESCENT, 0.5), 1.0)
	...
	stalker.draw_string(ThemeDB.fallback_font, Vector2(-GameConstants.TILE_SIZE, -GameConstants.TILE_SIZE * 1.625), state_name, HORIZONTAL_ALIGNMENT_LEFT, -1, 10, Palette.get_color(Palette.FLUORESCENT))
