...
## - 背の高い部品：種別の上半分 "<種別>#top" を 1 マス上の Overhead 層に置く（アクターより前に描かれ、木の梢が人物を隠す）
...
static func _apply_layer(field: FieldBase, layer: TileMapLayer, variants: Dictionary, tall: Dictionary) -> void:
	...
	for cell: Vector2i in cells:
		...
		if tall.has(type_name):
			var above: Vector2i = cell + Vector2i.UP
			if field.overhead.get_cell_source_id(above) == -1:
				field.overhead.set_cell(above, TileGenerator.SOURCE_ID, tall[type_name])


## 隣が同じ種別か。地図の外は「同じ」とみなし、端で切れ目を出さない
