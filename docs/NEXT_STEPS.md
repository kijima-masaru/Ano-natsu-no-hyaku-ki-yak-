...
| 3 | feat/truth-reveal | **PR 済・承認待ち**。8/30 夜：帰路の余韻 → F12 自宅前で澪が事実を並べる（選択肢 2 回、結末不変）→ 提示画面（1 件ずつ、暗転と無音、初回スキップ不可）→ `truth_revealed` → ナツ `msg_natsu_009` → 澪が去る → 真相版の町を歩き F01 で 8/30 が終わる |
| 4〜13 | feat/d31-endings 以降 | 未着手 |

### 次に何をすべきか
- タスク3 の PR をレビュー・マージ → `feat/d31-endings`（8/31、澪操作、F05 → F01 → F12 → F15 御渡橋、ED-A/B/C の分岐、事後の描写、`msg_natsu_010`、`ending_reached`）。
