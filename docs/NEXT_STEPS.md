...
| 6 | feat/end-game-flow | **PR 済・承認待ち**。`end_game` の本実装：`user://system.json` にクリア記録（到達 ED 一覧・回数・初回日時・最後の ED・ED 別回数）→ 相談窓口案内 → スタッフロール → タイトル。周回「裏面から」（下記）。はじめから時に接近度と怪異回数を初期化 |
| 7〜13 | feat/content-notice 以降 | 未着手 |

### 次に何をすべきか
- タスク6 の PR をレビュー・マージ → `feat/content-notice`（起動時警告の確定、相談窓口の一覧提示と確認、`docs/STORE_PAGE.md`）。
- **タスク6 の判断**：
