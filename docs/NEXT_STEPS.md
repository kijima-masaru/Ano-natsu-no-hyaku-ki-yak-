...
| 9 | fix/playtest-issues | **PR 済・承認待ち**。問題 10 件の対処：修正 3 件（全体図の視認性、操作案内の装置別表記 `InputDevice`、検証スロットの後始末）、誤報 1 件（棚卸しスクリプト）、仕様確認 1 件、タスク10 へ 2 件（接近度、屋内の暗さ）、タスク12 へ 1 件（未参照フラグ）、素材待ち 1 件、保留 1 件 |
| 10〜13 | chore/balance-tuning 以降 | 未着手 |

### 次に何をすべきか
- タスク9 の PR をレビュー・マージ → `chore/balance-tuning`。**先に目標プレイ時間を合意する**（ドライバ推定は最短 2 時間 40 分・全消費 4 時間、歩行が支配的）。接近度の加算と閾値（問題 #1）、追跡者の難度、暗所の視認性（問題 #7）、文字送り・音量の既定、daily だけの日の文。
